      ...
      if (read(dev_handler_, &recv_frames_[0], sizeof(recv_frames_[0])) >= 0)
      {
        for (int j = 0; j < 1; j++)
        {
          ...
          case 0x98C4D1EF:
          {
            ...
            msg.ctrl_fb_mode = 0x03 & recv_frames_[0].data[5] >> 4;      

            msg.ctrl_fb_RemoteSt = 0x01 & recv_frames_[0].data[5] >> 7;   
          ...
          case 0x98C4DAEF:
          {
            ...
            if (0x01 & recv_frames_[0].data[5])
              msg.io_fb_disCharge = true;
            else
              msg.io_fb_disCharge = false;
          ...
          case 0x98C4E2EF:
          {
            ...
            msg.bms_flag_Infor_charge_st = (recv_frames_[0].data[2] & 0xE0) >> 5;
          ...
          case 0x98C4EAEF:
          {
            ...
            msg.Veh_fb_LRDrvMCUFault = recv_frames_[j].data[4] & 0x3f;
            msg.Veh_fb_RRDrvMCUFault = (recv_frames_[j].data[4] >> 6) | ((recv_frames_[j].data[5] & 0x0f) << 2);

            if (0x10 & recv_frames_[j].data[5])
              msg.Veh_fb_AUXBMSDisOnline = true;
            else
              msg.Veh_fb_AUXBMSDisOnline = false;

            if (0x80 & recv_frames_[j].data[5])
              msg.Veh_fb_AuxRemoteDisOnline = true;
            else
              msg.Veh_fb_AuxRemoteDisOnline = false;

            // 辅件故障预留
            msg.Veh_fb_AuxReserve = recv_frames_[j].data[6] & 0x0f;
